...
function DetailApplication() {
  ...
  const handleAcceptandReject=async (id,action)=>{
    try{
    const response = await axios.post(`http://localhost:5000/api/applications/${id}`,{action})
...
//           <img alt="Profile photo" class="lg:w-1/2 w-full lg:h-auto h-64 object-cover  rounded" src={data?.user?.name}/>
